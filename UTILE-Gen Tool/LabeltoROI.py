...
# generate a system sepcific background image with the mean of bg pixels
def GetBG(maskpath, imagepath, projectname, only_realbg):
    try:
        os.mkdir('./Background/')
    except:
...
def generate_label_pool(maskpath: str, imagepath: str, projectname: str):
    ...
    new_mask_pool = f"./test_mask_pool_{projectname}"
    new_image_pool = f"./test_image_pool_{projectname}"

    particle_coords = {}

    # Create directoies to store the masks and images
